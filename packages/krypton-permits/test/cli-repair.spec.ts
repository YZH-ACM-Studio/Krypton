...
const permitsModel = new Proxy({
    async buildDriftReport(domainId: string) { return { domainId }; },
    async repairOrphanProblemLock(...args: any[]) { calls.orphan.push(args); },
    async repairFenceWithoutProblemLock(...args: any[]) { calls.fenceWithoutLock.push(args); },
    async repairErroredProblemWriteClaim(...args: any[]) { calls.writeClaim.push(args); },
    async recoverActiveProblemWriteClaim(...args: any[]) { calls.activeWriteClaim.push(args); },
    async repairAclMutation(...args: any[]) { calls.aclMutation.push(args); },
}, {
...
Module._load = function load(request: string, parent: NodeModule, isMain: boolean) {
    if (request === '@hydrooj/utils') {
        return { Logger: class Logger { success() { } } };
    }
...
function getRepairAction() {
    ...
    const chain: any = {
        option() { return chain; },
        action(callback: any) { action = callback; return chain; },
    };
    registerCommands({
        cli: {
            command(name: string) {
                if (name.startsWith('permits:repair ')) return chain;
                return { ...chain, action() { return chain; } };
            },
        },
    });
    return action;
...
describe('ACL repair CLI', () => {
    it('requires explicit partial storage/metadata inspection before repairing an ERROR write claim', async () => {
        ...
        const base = {
            requestId: 'claim-1', actor: '7', confirm: 'REPAIR_ACL',
        };
        ...
        await action('problem-write-claim', 'system', '12', undefined, {
            ...base, writeClaimCheck: 'PARTIAL_WRITE_INSPECTED',
        });
    ...
    it('keeps the ERROR token compatible and routes only the process-quiesced token to ACTIVE recovery', async () => {
        const action = getRepairAction();
        const base = {
            requestId: 'claim-active', actor: '7', confirm: 'REPAIR_ACL',
        };
        ...
        expect(calls.writeClaim.slice(errorRepairsBefore)).to.deep.equal([
            ['system', 15, 'claim-active'],
        ]);
        expect(calls.activeWriteClaim).to.deep.equal([]);

        await action('problem-write-claim', 'system', '15', undefined, {
            ...base, writeClaimCheck: 'PROCESS_QUIESCED_AND_PARTIAL_WRITE_INSPECTED',
        });
        expect(calls.activeWriteClaim).to.deep.equal([[
            'system', 15, 'claim-active', 'PROCESS_QUIESCED_AND_PARTIAL_WRITE_INSPECTED',
        ]]);
    });
    ...
    it('repairs an orphan lock only through the pair-scoped exact requestId entrypoint', async () => {
        ...
        await action('orphan-problem-lock', 'system', '13', '99', {
            requestId: 'orphan-13', actor: '7', confirm: 'REPAIR_ACL',
        });
    ...
    it('repairs a fence without a ProblemDoc lock through its exact requestId entrypoint', async () => {
        ...
        await action('fence-without-problem-lock', 'system', '14', '100', {
            requestId: 'fence-14', actor: '7', confirm: 'REPAIR_ACL',
        });
    ...
    it('repairs an ACL mutation only through its pair-scoped exact requestId entrypoint', async () => {
        ...
        await action('acl-mutation', 'system', '16', '101', {
            requestId: 'acl-16', actor: '7', confirm: 'REPAIR_ACL',
        });
