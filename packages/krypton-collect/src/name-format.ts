...
import type { CollectAllowedExt, CollectSlot } from './types';
...
function tokenValue(token: CollectNameToken, ctx: CollectNameContext): string {
    const studentId = ctx.studentId?.trim() || `unbound-UID${ctx.uid}`;
...
export function renderAssignedFileName(template: string, ctx: CollectNameContext): string {
    ...
    const replaced = resolved.replace(new RegExp(TOKEN_RE.source, 'g'), (_all, token: string) => (
        ALLOWED_TOKENS.has(token) ? tokenValue(token as CollectNameToken, ctx) : ''
    ));
...
export function slotPreviewExt(slot: Pick<CollectSlot, 'allowedExt'>): CollectAllowedExt {
