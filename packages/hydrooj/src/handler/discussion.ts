...
export class DiscussionDetailHandler extends DiscussionHandler {
    ...
    async get(domainId: string, did: ObjectId, page = 1) {
        ...
        const replyPermissions = Object.fromEntries(
            drdocs.map((drdoc) => {
                const canEdit = this.user.own(drdoc) && this.user.hasPerm(PERM.PERM_EDIT_DISCUSSION_REPLY_SELF);
                const canDelete =
                    (this.user.own(drdoc) && this.user.hasPerm(PERM.PERM_DELETE_DISCUSSION_REPLY_SELF)) ||
        ...
        this.response.body = {
            ...
            permissions: {
                canEditDiscussion:
                    this.user.hasPerm(PERM.PERM_EDIT_DISCUSSION) || (this.user.own(this.ddoc) && this.user.hasPerm(PERM.PERM_EDIT_DISCUSSION_SELF)),
                canDeleteDiscussion:
