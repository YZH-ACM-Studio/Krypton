...
function ObjectiveEditorShell({
  ...
  useEffect(() => {
    if (JSON.stringify(config) !== initialConfigRef.current) dirtyRef.current = true;
  }, [config]);
      ...
      >
        ...
        <aside className="space-y-5 lg:border-l lg:border-border/70 lg:pl-5">
          ...
          <label className="block space-y-1.5">
            ...
            <Input name="difficulty" type="number" min={1} max={10} defaultValue={pdoc.difficulty || ''} className="min-h-11" disabled={locked} />
          </label>
          {isCreate ? (
            <p className="border-y border-border/70 py-3 text-xs text-muted-foreground">新题首次保存固定为隐藏，检查后可在编辑页发布。</p>
          ) : (
