...
3. 在本地完成确定性的预处理：识别赛事、年份、场次和题号；按用户给出的筛选条件或完整题号清单选择题目；解析题面、公式、图片和 Hydro 兼容的样例格式；解包并配对输入输出文件；为每道题生成显式测试用例配置；准备来源、PID、系统标签、知识导图标签、出题人和待挂训练信息。只有用户提供或可核验的赛时数据才录入，缺失时不得伪造 `0/0`。
