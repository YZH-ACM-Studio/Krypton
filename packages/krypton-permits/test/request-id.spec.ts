...
describe('ACL HTTP request id', () => {
    it('derives the same retryable id from the mutation identity when the client omits one', () => {
        ...
        expect(deriveAclRequestId(undefined, 'grant', ...input)).to.equal(
            deriveAclRequestId(undefined, 'grant', ...input),
        );
        expect(deriveAclRequestId(undefined, 'grant', ...input)).not.to.equal(
            deriveAclRequestId(undefined, 'grant', 'system', 42, 8, 'maintainer'),
        );
    });
