...
describe('P4.5 Vigil all-contests contracts', () => {
    ...
    it('renders the searchable all-contests tab and reuses the existing detail route', () => {
        ...
        expect(page).to.match(/fetchHydroResponse\(`\/api\/admin\/vigil\/contests\?\$\{params\}`/);
        expect(page).to.include("readHydroResponseError(response, '加载比赛列表失败')");
        expect(page).to.include("headers: { Accept: 'application/json' }");
    ...
    it('clears stale error variants before each Vigil request', () => {
        ...
        expect(hook).to.include("else setErr(e instanceof Error && e.message ? e.message : '加载失败')");
    });
});
