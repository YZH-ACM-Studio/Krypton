...
describe('p3.9 dedicated basic objective editors', () => {
  it('registers every route-template-component tuple without GenericPage fallback', () => {
    ...
    expect(readFileSync(resolve(root, 'src/pages/basic-objective-editors.tsx'), 'utf8')).not.to.include("type BasicKind = 'single'");
  });
  ...
  it('keeps fields independent while sharing only the neutral editor shell', () => {
    ...
    for (const component of ['SingleProblemEditorPage', 'MultiProblemEditorPage', 'TrueFalseProblemEditorPage', 'BlankProblemEditorPage']) {
