...
describe('P3.7 redemption manage capability', () => {
  ...
  it('registers manage and redeem pages behind server capability', () => {
    ...
    const dialog = readFileSync(resolve(root, 'src/components/redeem-dialog.tsx'), 'utf8');
    const router = readFileSync(resolve(root, 'src/router.tsx'), 'utf8');
    const settings = readFileSync(resolve(root, 'src/pages/user-account.tsx'), 'utf8');
    ...
    expect(sidebar).not.to.match(/label: '兑换',\s*href: '\/redeem'/);
    expect(dialog).to.include("fetchHydroResponse('/redeem'");
    expect(router).to.include('RedeemDialogButton');
    expect(settings).to.include('RedeemDialogButton');
