...
function assertSnapshot(value: unknown): asserts value is MindmapSnapshot {
  ...
  if (!snapshot || !Array.isArray(snapshot.nodes) || !snapshot.config || typeof snapshot.config !== 'object') {
...
export async function loadNodeProblems(nodeId: string, admin: boolean): Promise<PanelProblem[]> {
  const endpoint = admin ? '/api/mindmap/admin/node-problems' : '/api/mindmap/problems';
  const response = await fetch(`${endpoint}?nodeId=${encodeURIComponent(nodeId)}`, { headers: { Accept: 'application/json' } });
...
export async function searchMindmapProblems(query: string, signal?: AbortSignal): Promise<ProblemOption[]> {
  const response = await fetch(`/api/mindmap/admin/problems?q=${encodeURIComponent(query)}`, {
    headers: { Accept: 'application/json' },
    signal,
  });
