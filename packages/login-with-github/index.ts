        ...
        ctx.oauth.provide('github', {
            ...
            callback: async function callback({ state, code }) {
                ...
                const res = await superagent
                    .post(`${config.endpoint || 'https://github.com'}/login/oauth/access_token`)
                    .send({
                        client_id: config.id,
                ...
                const userInfo = await superagent
                    .get(`${config.endpoint ? `${config.endpoint}/api` : 'https://api.github.com'}/user`)
                ...
                if (!ret.email) {
                    const emailInfo = await superagent
                        .get(`${config.endpoint ? `${config.endpoint}/api` : 'https://api.github.com'}/user/emails`)
                        .set('User-Agent', 'Hydro-OAuth')
                        .set('Accept', 'application/vnd.github.v3+json')
                        .set('Authorization', `token ${t}`);
