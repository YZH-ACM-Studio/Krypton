...
function TaskGraphNodeComponent({ data, selected }: { data: NodeDataShape; selected?: boolean }) {
  ...
  return (
    <>
      ...
      {!isEnd && <Handle type="source" position={Position.Right} className="!size-2 !border-2" />}
    </>
  );
}
...
function TaskGraphInner({ graph, presets, progress, onChange, selectedNodeId, onNodeSelect, height, extraControls }: TaskGraphRendererProps) {
  ...
  const [rfNodes, setRfNodes] = useState<RFNode<NodeDataShape>[]>(() => toRFNodes(graph, presetMap, progress));
  ...
  const [rfEdges, setRfEdges] = useState<Edge[]>(() => toRFEdges(graph, progress, selectedEdgeIds));
      ...
      JSON.stringify({
        ...
        pk: progress
          ? Object.keys(progress)
              .sort()
              .map((k) => `${k}:${progress[k].completed ? 1 : 0}:${progress[k].current}/${progress[k].target}${progress[k].overridden ? '*' : ''}`)
              .join(',')
          : '',
      }),
    [graph, progress],
  ...
  const onNodesChange = useCallback(
    (changes: NodeChange<RFNode<NodeDataShape>>[]) => {
