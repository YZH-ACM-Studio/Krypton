...
const calls = {
    resumeMarkers: [] as any[], revokePairs: [] as any[], revokeSource: [] as any[], writes: [] as any[],
};
const permitsColl = {
    async findOne(filter: any) { return rows.find((row) => matches(row, filter)) || null; },
    find(filter: any) {
        const cursor = {
            sort() { return cursor; },
            async toArray() { return rows.filter((row) => matches(row, filter)); },
        };
        return cursor;
    },
};
const aclService = {
    ...
    async revokePairs(...args: any[]) { calls.revokePairs.push(args); return 1; },
    async revokeSource(...args: any[]) { calls.revokeSource.push(args); return true; },
    async resumeMarkersForPair(...args: any[]) { calls.resumeMarkers.push(args); return { role: 'maintainer' }; },
};
...
require.cache[repositoryPath] = {
    ...
    exports: { mongoAclRepository: { async getSources() { return []; } } },
} as NodeModule;
...
describe('legacy canonical model compatibility', () => {
    it('lists legacy direct/contest verifier/maintainer rows and normalizes active in memory', async () => {
        ...
        expect(byProblem.map((row) => [row.pid, row.role, row.active])).to.deep.equal([
            [3, 'verifier', true],
        ]);
    ...
    it('preserves legacy viaContest provenance for source-constrained revoke before repair', async () => {
        expect(await model.revoke('system', ids.directVerifier as any, {
            requireOwner: 'direct', requestId: 'old-direct',
        })).to.equal(true);
        expect(await model.revoke('system', ids.contestVerifier as any, {
            requireOwner: 'viaContest', viaContest: new FakeObjectId('c1') as any, requestId: 'old-contest',
        })).to.equal(true);
        expect(await model.revoke('system', ids.contestMaintainer as any, {
            requireOwner: 'viaContest', viaContest: new FakeObjectId('other') as any, requestId: 'wrong-contest',
        })).to.equal(false);
    ...
    it('delegates resumeFence to pair-marker recovery so an orphan ProblemDoc lock is not ignored', async () => {
        ...
        expect(calls.resumeMarkers).to.deep.equal([
            [{ domainId: 'system', pid: 9, uid: 99 }, 'orphan-request'],
        ]);
    });
});
