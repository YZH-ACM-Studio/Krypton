...
const getRecordDetail = (data) => {
  if (!shouldShowDetail(data)) {
    return <span className={`record-status--text ${STATUS_CODES[data.status]}`}>{STATUS_TEXTS[data.status]}</span>;
  }
  const stat = _.pick(_.groupBy(data.testCases || [], 'status'), _.keys(STATUS_SCRATCHPAD_SHORT_TEXTS));
