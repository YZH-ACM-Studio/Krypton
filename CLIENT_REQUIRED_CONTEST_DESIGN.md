...
封锁受众按 canonical 比赛事实判定：

- 显式学校、用户组或 legacy `assign` 范围：在封锁窗口开始时提前封锁同时命中范围的用户。
- 真正不限参赛的个人赛（无学校/用户组、无 `assign`、无邀请码）：只有已通过 Client 自动参加并存在 canonical `ContestStatus.attend` 的用户会被封锁。
- 带邀请码的个人赛：继续只封锁已兑换邀请码并 attend 的用户。
- 团队赛：只封锁已定版比赛中 active `ContestTeam` 的成员；仅预绑定但未定版的批次不产生封锁受众。
- Client 自动 attend 成功后必须立即精确失效该 `(domainId, uid)` 的封锁缓存；团队定版、直接建队和成员变化在仍持有同一比赛轻量边界时同步失效该域缓存，随后才执行邀请清理或 Vigil 网络调用。每次失效同时推进进程内 generation，异步旧计算不得重新写回。退出 Client 不撤销 attend，普通浏览器保持封锁至窗口结束。

不会封锁：

- 使用普通浏览器 session 的管理员。
- 比赛 owner。
- 比赛 maintainer。
- 系统/评测相关特权用户。
- 不在该比赛参赛范围内的其他学生。
...
有效 Client session 不是上述“普通浏览器不封锁”的旁路，也不是主 OJ 的全站通行证。它只能访问自身绑定 `domainId` 和 `contestId` 的 `/exam-mode/:tid`、`/paper/:tid`，以及携带同一 `tid`、仍由下游做比赛归属校验的提交、题面附件和评测轮询端点。任何首页、全站记录、其它域、其它比赛或其它 `tid` 请求都必须保留 Client session 并重定向回绑定工作台；该限制同样作用于通过 Client 进入的管理员。拦截在 Hydro 已选定 handler、但路由 `prepare` 和业务方法尚未执行的 `handler/before-prepare` 阶段完成，避免被拒绝路由发生读写，也避免无 handler 的早返回被框架改写成 500。考试壳内普通 HTML 表单的可预期 4xx 错误必须留在考试壳，不得渲染带主站导航的通用错误页。
