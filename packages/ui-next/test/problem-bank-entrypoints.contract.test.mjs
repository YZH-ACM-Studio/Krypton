...
it('bootstrap exposes one fail-closed observable problem-bank capability', () => {
...
it('sidebar consumes only canBrowseProblemBank for all enumeration entries', () => {
...
it('unified problem bank pushes filters into the canonical author scope', () => {
...
it('creation hub and all eight routes consume the shared kind mapping', () => {
...
it('legacy composite authoring runtime files are deleted without an alias', () => {
...
it('manual grading derives the authoritative domain before every domain-sensitive operation', () => {
...
it('tasks problem search is capability-gated and pushes the canonical scope into Mongo', () => {
...
it('task graph writes validate new problem refs before every audit or task write and grandfather old refs', () => {
...
it('task clone validates every copied problem ref as a new container reference', () => {
...
it('contest writes validate selection before create/edit and retain existing references', () => {
...
it('homework writes validate selection before create/edit and retain existing references', () => {
...
it('training DAG writes validate selection before create/edit and grandfather the old DAG union', () => {
...
it('course DAG writes validate selection before create/edit and grandfather the old DAG union', () => {
...
it('contest detail base ignores forged method domains before contest and status reads', () => {
