...
export async function ensureIndexes(): Promise<void> {
    ...
    await Promise.all(
        [patScoreColl, gpltScoreColl, cspScoreColl].map(async (coll) => {
            try {
                const existing = await coll.indexes();
                await Promise.all(
                    existing
                        .filter((i) => i.name !== '_id_' && i.key && Object.hasOwn(i.key, 'userId'))
                        .map((i) =>
                            coll.dropIndex(i.name as string).catch(() => {
                                /* concurrent drop / gone */
                            }),
                        ),
                );
            } catch {
                /* collection not created yet — nothing to drop */
            }
        }),
    );
    ...
    await Promise.all([
        ...
        patScoreColl.createIndex({ domainId: 1, studentDocId: 1, level: 1, year: 1, season: 1 }, { unique: true }),
        patScoreColl.createIndex({ domainId: 1, level: 1, year: 1 }),

        gpltScoreColl.createIndex({ domainId: 1, studentDocId: 1, level: 1, year: 1 }, { unique: true }),
        ...
        stayEventsColl.createIndex({ domainId: 1, userId: 1, source: 1 }, { unique: true }),
