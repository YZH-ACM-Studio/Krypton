  ...
  return (
    <article className="group grid gap-3 border-b border-border/70 py-4 last:border-b-0 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {course.term ? <Badge variant="outline" className="rounded-md text-[10px]">{course.term}</Badge> : null}
            ...
            <p className="mt-1.5 flex flex-wrap items-center gap-3 text-[11px] tabular-nums text-muted-foreground">
              ...
              {(course.courseGroupIds || []).length ? (
                <span className="inline-flex items-center gap-1"><Users className="size-3" />限指定班级</span>
              ) : <span>全域可见</span>}
            </p>
          </div>
        </div>
      ...
      <div className="flex items-center justify-end gap-1">
        {manageable ? <Button asChild variant="ghost" size="sm"><a href={`/course/${tid}/edit`}>编辑</a></Button> : null}
        <Button asChild variant="ghost" size="icon" className="size-10">
          <a href={`/course/${tid}`} aria-label={`打开${course.title}`}><ChevronRight className="size-4" /></a>
        </Button>
      </div>
    </article>
  );
  ...
  return (
    <section aria-labelledby={`course-section-${title}`}>
      <div className="mb-2 flex items-end justify-between gap-4">
        <div>
          <h2 id={`course-section-${title}`} className="text-sm font-semibold">{title}</h2>
  ...
  return (
    <main className="mx-auto w-full max-w-6xl space-y-8 pb-8">
      <header className="flex flex-col gap-4 border-b border-border/70 pb-6 sm:flex-row sm:items-end sm:justify-between">
        ...
        {data.canCreate ? <Button asChild className="min-h-11 gap-1.5"><a href="/course/create"><Plus className="size-4" />新建课程</a></Button> : null}
      </header>
      ...
      <form method="get" action="/course" className="flex flex-col gap-2 sm:flex-row" role="search">
        ...
        <Button type="submit" variant="outline" className="min-h-11 px-5">搜索</Button>
      </form>
      ...
      {!courses.length ? (
        ...
        <div className="space-y-10">
          <CourseSection title="我的管理" description="你创建或有权维护的课程" courses={managed} statuses={data.tsdict || {}} managedIds={managedIds} />
          <CourseSection title="我可学习" description="当前账号可进入的课程" courses={learnable} statuses={data.tsdict || {}} managedIds={managedIds} />
        </div>
      )}
