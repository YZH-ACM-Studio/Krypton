...
export default function reducer(
  ...
  switch (action.type) {
    ...
    case 'CONFIG_AUTOCASES_UPDATE': {
      ...
      if (subtasks.length === 0) next.subtasks = [];
        ...
        next.subtasks = subtasks.map((subtask) => ({
          ...subtask,
          ...{ cases: subtask.cases.map((i) => ({ input: i.input, output: i.output })) },
        }));
      }
      return next;
    }
    ...
    case 'problemconfig/moveTestcases': {
      ...
      const subtasks = cloneDeep(state.subtasks);
      const targetCases = action.payload.target === -1 ? state.__cases : subtasks.find((s) => s.id === action.payload.target)?.cases || [];
