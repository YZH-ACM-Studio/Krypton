...
import type {
    AclMutationFence, AclPair, CanonicalPermit, PermitSource,
...
class MemoryServiceRepository implements AclServiceRepository {
    ...
    async getCanonical(pair: AclPair) { return this.canonical.get(pairKey(pair)) || null; }
    async getSources(pair: AclPair) { return (this.sources.get(pairKey(pair)) || []).map((x) => ({ ...x })); }
    async getFence(pair: AclPair) { return this.fences.get(pairKey(pair)) || null; }
    async getProblemAclMutationLock(pair: AclPair) { return this.problemLocks.get(pairKey(pair)) || null; }
    ...
    async applySource(fence: AclMutationFence) {
        ...
        if (fence.intent.role) {
            next.push({
                domainId: fence.domainId, pid: fence.pid, uid: fence.uid,
                sourceType: fence.intent.sourceType, sourceId: fence.intent.sourceId,
                role: fence.intent.role, active: true, grantedBy: fence.intent.grantedBy,
                grantedAt: new Date('2026-07-11T00:00:00.000Z'), note: fence.intent.note,
            });
        }
    ...
    async listSourcesForContest(domainId: string, sourceId: string, uid?: number) {
        return [...this.sources.values()].flat().filter((x) => x.domainId === domainId
            && x.sourceType === 'contest' && x.sourceId === sourceId
            && (uid === undefined || x.uid === uid));
    }
    ...
    async listCanonicalForContest(domainId: string, sourceId: string, uid?: number) {
        return [...this.canonical.values()].filter((x) => x.domainId === domainId
            && x.active === true && x.viaContest === sourceId
            && (uid === undefined || x.uid === uid));
    }
...
describe('ACL service', () => {
    ...
    async function expectClaimBoundDedicatedRepairsFailClosed(options: {
        ...
        if (options.currentClaim) {
            repo.problemWriteClaims.set(`system:${options.pid}`, {
                domainId: 'system', pid: options.pid,
                requestId: options.currentClaim.requestId,
                actor: 1, operation: 'permit-update', state: options.currentClaim.state,
        ...
        await Promise.all(entries.map(async (entry) => {
            ...
            const error = await (entry.kind === 'orphan'
            ...
            expect(error.message).to.contain(options.message);
    ...
    it('publishing clears every verifier source while preserving direct and contest maintainers', async () => {
        ...
        repo.canonical.set(pairKey(staleVerifier), {
            ...staleVerifier, role: 'verifier', active: true, grantedBy: 1,
            grantedAt: new Date(), viaContest: null, note: 'stale-without-source',
        });
    ...
    it('removing one contest source never removes another contest or direct grant', async () => {
        ...
        repo.canonical.set(pairKey(staleContestPair), {
            ...staleContestPair, role: 'verifier', active: true, grantedBy: 1,
            grantedAt: new Date(), viaContest: 'c1', note: 'legacy-contest-canonical',
        });
    ...
    it('batch revoke clears all sources for each requested pair without crossing domains', async () => {
        ...
        repo.canonical.set(pairKey(stalePair), {
            ...stalePair, role: 'maintainer', active: true, grantedBy: 1,
            grantedAt: new Date(), viaContest: null, note: 'stale',
        });
        repo.mirrors.set('system:6', new Set([60]));

        const removed = await service.revokePairs(
            'system', [{ pid: 4, uid: 60 }, { pid: 5, uid: 60 }, { pid: 6, uid: 60 }], 'batch-r', 1,
        );
    ...
    it('preload excludes fenced pairs and separates maintained from all permitted pids', async () => {
        ...
        repo.fences.set(pairKey({ domainId: 'system', pid: 6, uid: 70 }), {
            domainId: 'system', pid: 6, uid: 70, requestId: 'stuck', from: 'maintainer', to: null,
            intent: { sourceType: 'direct', sourceId: 'direct', role: null, grantedBy: 1, note: '' },
            completedSteps: [], lastError: 'stuck',
            createdAt: new Date(), updatedAt: new Date(),
        });
    ...
    it('preload denies an orphan ProblemDoc lock even when fence insertion never completed', async () => {
        ...
        repo.problemLocks.set(pairKey({ domainId: 'system', pid: 8, uid: 80 }), {
            domainId: 'system', pid: 8, uid: 80, requestId: 'orphan-lock',
            from: 'maintainer', to: 'verifier',
            intent: {
                action: 'set-source', sourceType: 'direct', sourceId: 'direct',
                role: 'verifier', grantedBy: 1, note: '',
            },
            createdAt: new Date(), updatedAt: new Date(),
        });
    ...
    it('reports orphan ProblemDoc locks, fence/lock mismatches, and active/error write claims read-only', async () => {
        const orphan = {
            domainId: 'system', pid: 40, uid: 400, requestId: 'orphan-40',
            from: null, to: 'maintainer' as const,
            intent: {
                action: 'set-source' as const, sourceType: 'direct' as const, sourceId: 'direct',
                role: 'maintainer' as const, grantedBy: 1, note: '',
            },
            createdAt: new Date(), updatedAt: new Date(),
        };
        repo.problemLocks.set(pairKey(orphan), orphan);
        const fenceOnly = {
            domainId: 'system', pid: 41, uid: 410, requestId: 'fence-41',
            from: 'verifier' as const, to: null,
            intent: {
                action: 'set-source' as const, sourceType: 'direct' as const, sourceId: 'direct',
                role: null, grantedBy: 1, note: '',
            },
            completedSteps: [], lastError: 'stuck',
            createdAt: new Date(), updatedAt: new Date(),
        };
        ...
        repo.problemWriteClaims.set('system:42', {
            domainId: 'system', pid: 42, requestId: 'claim-active', actor: 1,
            operation: 'files-upload', state: 'active', lastError: null,
            createdAt: new Date(), updatedAt: new Date(),
        });
        repo.problemWriteClaims.set('system:43', {
            domainId: 'system', pid: 43, requestId: 'claim-error', actor: 1,
            operation: 'metadata-edit', state: 'error', lastError: 'failed',
            createdAt: new Date(), updatedAt: new Date(),
        });
        ...
        expect(report.fenceProblemLockMismatches.map((row) => ({
            pid: row.pid, reasons: row.reasons,
        }))).to.deep.equal([{ pid: 41, reasons: ['missing-problem-lock'] }]);
        expect(report.aclMutationFences.map((row) => ({
            pid: row.pid,
    ...
    it('repairs an orphan lock only with its exact original request and resumes failures idempotently', async () => {
        ...
        repo.problemLocks.set(pairKey(pair), {
            ...pair, requestId: 'orphan-44', from: null, to: 'maintainer',
            intent: {
                action: 'set-source', sourceType: 'direct', sourceId: 'direct',
        ...
        const conflict = await service.repairOrphanProblemLock(pair, 'other-request')
            .catch((error) => error);
        ...
        const first = await service.repairOrphanProblemLock(pair, 'orphan-44')
            .catch((error) => error);
    ...
    it('refuses orphan-lock repair when the ProblemDoc disappeared and preserves the marker', async () => {
        ...
        repo.problemLocks.set(pairKey(pair), {
            ...pair, requestId: 'orphan-45', from: null, to: null,
            intent: {
                action: 'reconcile', sourceType: 'direct', sourceId: 'direct',
                role: null, grantedBy: 1, note: '',
            },
            createdAt: new Date(), updatedAt: new Date(),
        });
        ...
        const error = await service.repairOrphanProblemLock(pair, 'orphan-45')
            .catch((caught) => caught);
    ...
    it('repairs a fence without a ProblemDoc lock only with its exact requestId', async () => {
        ...
        repo.fences.set(pairKey(pair), {
            ...pair, requestId: 'fence-50', from: null, to: 'maintainer',
            intent: {
                action: 'set-source', sourceType: 'direct', sourceId: 'direct',
                role: 'maintainer', grantedBy: 1, note: '',
            },
            completedSteps: [], lastError: 'clear-order crash',
            createdAt: new Date(), updatedAt: new Date(),
        });

        const wrong = await service.repairFenceWithoutProblemLock(pair, 'other-request')
            .catch((error) => error);
    ...
    it('routes an ERROR-claim-bound fence through problem-write-claim repair', async () => {
        ...
        repo.problemWriteClaims.set('system:51', {
            domainId: 'system', pid: 51, requestId: 'claim-51', actor: 1,
            operation: 'permit-update', state: 'error', lastError: 'failed',
            createdAt: new Date(), updatedAt: new Date(),
        });
        repo.fences.set(pairKey(pair), {
            ...pair, requestId: 'fence-51', from: null, to: 'maintainer',
            intent: {
                action: 'set-source', sourceType: 'direct', sourceId: 'direct',
                role: 'maintainer', grantedBy: 1, note: '',
            },
            completedSteps: [], lastError: 'failed',
            createdAt: new Date(), updatedAt: new Date(),
            writeClaimRequestId: 'claim-51',
        });

        const error = await service.repairFenceWithoutProblemLock(pair, 'fence-51')
            .catch((caught) => caught);
    ...
    it('converges both fence-only and lock-only markers before a global write claim', async () => {
        ...
        const marker = (pair: AclPair, requestId: string) => ({
            ...pair, requestId, from: null, to: 'maintainer' as const,
            intent: {
                action: 'set-source' as const, sourceType: 'direct' as const, sourceId: 'direct',
                role: 'maintainer' as const, grantedBy: 1, note: '',
            },
    ...
    it('does not touch ACL markers while another global write claim exists', async () => {
        ...
        repo.problemWriteClaims.set('system:53', {
            domainId: 'system', pid: 53, requestId: 'claim-53', actor: 1,
            operation: 'metadata-edit', state: 'active', lastError: null,
            createdAt: new Date(), updatedAt: new Date(),
        });
        repo.fences.set(pairKey(pair), {
            ...pair, requestId: 'fence-53', from: null, to: null,
            intent: {
                action: 'reconcile', sourceType: 'direct', sourceId: 'direct',
                role: null, grantedBy: 1, note: '',
            },
        ...
        const error = await service.prepareProblemWriteClaim('system', 53)
            .catch((caught) => caught);
    ...
    it('reactivates an exact ERROR write claim, converges its bound fence, then clears the claim', async () => {
        const claim = {
            domainId: 'system', pid: 46, requestId: 'claim-46', actor: 1,
            operation: 'permit-update', state: 'error', lastError: 'outer failed',
            createdAt: new Date(), updatedAt: new Date(),
        };
        repo.problemWriteClaims.set('system:46', claim);
        const pair = { domainId: 'system', pid: 46, uid: 460 };
        repo.fences.set(pairKey(pair), {
            ...pair, requestId: 'acl-46', from: 'maintainer', to: 'maintainer',
            intent: {
                action: 'reconcile', sourceType: 'direct', sourceId: 'direct',
                role: null, grantedBy: 1, note: '',
            },
            completedSteps: ['source', 'canonical', 'mirror', 'verify'],
            lastError: 'clearFence failed', createdAt: new Date(), updatedAt: new Date(),
    ...
    it('refuses ACTIVE write-claim recovery without both the exact requestId and process-quiesced confirmation', async () => {
        repo.problemWriteClaims.set('system:54', {
            domainId: 'system', pid: 54, requestId: 'claim-54', actor: 1,
            operation: 'files-upload', state: 'active', lastError: null,
            createdAt: new Date(), updatedAt: new Date(),
        });

        const missingConfirmation = await service.recoverActiveProblemWriteClaim(
            'system', 54, 'claim-54', 'PARTIAL_WRITE_INSPECTED',
        ).catch((error: Error) => error);
        expect(missingConfirmation.message).to.contain(
            'PROCESS_QUIESCED_AND_PARTIAL_WRITE_INSPECTED',
        );
        expect(repo.problemWriteClaims.get('system:54')?.state).to.equal('active');

        const wrongRequest = await service.recoverActiveProblemWriteClaim(
            'system', 54, 'other-claim', 'PROCESS_QUIESCED_AND_PARTIAL_WRITE_INSPECTED',
        ).catch((error: Error) => error);
        expect(wrongRequest.message).to.contain('requestId mismatch');
    ...
    it('recovers a crash-left ACTIVE write claim through ERROR repair and leaves the problem writable', async () => {
        repo.problemWriteClaims.set('system:55', {
            domainId: 'system', pid: 55, requestId: 'claim-55', actor: 1,
            operation: 'permit-update', state: 'active', lastError: null,
            createdAt: new Date(), updatedAt: new Date(),
        });
        repo.failOnceAt = 'source';
        const failedMutation = await service.grantDirect(
            'system', 55, 550, 'maintainer', 1, 'acl-55', '', 'claim-55',
        ).catch((error) => error);
        ...
        await service.recoverActiveProblemWriteClaim(
            'system', 55, 'claim-55', 'PROCESS_QUIESCED_AND_PARTIAL_WRITE_INSPECTED',
        );
    ...
    it('keeps the recovered claim in ERROR when an ACL marker belongs to another claim', async () => {
        ...
        repo.problemWriteClaims.set('system:56', {
            domainId: 'system', pid: 56, requestId: 'claim-56', actor: 1,
            operation: 'permit-update', state: 'active', lastError: null,
            createdAt: new Date(), updatedAt: new Date(),
        });
        const marker = {
            ...pair, requestId: 'acl-56', from: null, to: 'maintainer' as const,
            intent: {
                action: 'set-source' as const, sourceType: 'direct' as const, sourceId: 'direct',
                role: 'maintainer' as const, grantedBy: 1, note: '',
            },
        ...
        repo.fences.set(pairKey(pair), {
            ...marker, completedSteps: [], lastError: 'crashed',
        });

        const error = await service.recoverActiveProblemWriteClaim(
            'system', 56, 'claim-56', 'PROCESS_QUIESCED_AND_PARTIAL_WRITE_INSPECTED',
        ).catch((caught: Error) => caught);

        expect(error.message).to.contain('not bound to write claim claim-56');
    ...
    it('fails closed when the ACTIVE-to-ERROR recovery CAS loses a race', async () => {
        repo.problemWriteClaims.set('system:57', {
            domainId: 'system', pid: 57, requestId: 'claim-57', actor: 1,
            operation: 'metadata-edit', state: 'active', lastError: null,
            createdAt: new Date(), updatedAt: new Date(),
        });
        ...
        const error = await service.recoverActiveProblemWriteClaim(
            'system', 57, 'claim-57', 'PROCESS_QUIESCED_AND_PARTIAL_WRITE_INSPECTED',
        ).catch((caught: Error) => caught);

        expect(error.message).to.contain('changed before exact ACTIVE recovery');
    ...
    it('never clears an active/wrong claim and restores ERROR when bound ACL resume fails', async () => {
        repo.problemWriteClaims.set('system:49', {
            domainId: 'system', pid: 49, requestId: 'claim-49', actor: 1,
            operation: 'metadata-edit', state: 'error', lastError: 'failed',
            createdAt: new Date(), updatedAt: new Date(),
        });
        const wrong = await service.repairErroredProblemWriteClaim('system', 49, 'other-claim')
            .catch((error) => error);
        expect(wrong.message).to.contain('requestId mismatch');
        expect(repo.problemWriteClaims.get('system:49')?.state).to.equal('error');

        repo.problemWriteClaims.set('system:47', {
            domainId: 'system', pid: 47, requestId: 'claim-47', actor: 1,
            operation: 'permit-update', state: 'active', lastError: null,
            createdAt: new Date(), updatedAt: new Date(),
        });
        const active = await service.repairErroredProblemWriteClaim('system', 47, 'claim-47')
            .catch((error) => error);
        ...
        repo.problemWriteClaims.set('system:48', {
            domainId: 'system', pid: 48, requestId: 'claim-48', actor: 1,
            operation: 'permit-update', state: 'error', lastError: 'outer failed',
            createdAt: new Date(), updatedAt: new Date(),
        });
        repo.problemLocks.set(pairKey(pair), {
            ...pair, requestId: 'acl-48', from: null, to: 'maintainer',
            intent: {
                action: 'set-source', sourceType: 'direct', sourceId: 'direct',
                role: 'maintainer', grantedBy: 1, note: '',
            },
        ...
        const failed = await service.repairErroredProblemWriteClaim('system', 48, 'claim-48')
            .catch((error) => error);
    ...
    it('reports three legacy drift categories plus source/canonical conflicts read-only', async () => {
        ...
        repo.sources.set(pairKey(conflictPair), [{
            ...conflictPair, sourceType: 'direct', sourceId: 'direct', role: 'maintainer',
            active: true, grantedBy: 1, grantedAt: new Date(), note: '',
        }]);
        repo.canonical.set(pairKey(conflictPair), {
            ...conflictPair, role: 'verifier', active: true, grantedBy: 1,
            grantedAt: new Date(), viaContest: null, note: '',
        });
        ...
        expect(report.canonicalMaintainerWithoutLegacy).to.deep.equal([
            { domainId: 'system', pid: 11, uid: 110 },
        ]);
        expect(report.verifierInLegacy).to.deep.equal([
            { domainId: 'system', pid: 10, uid: 102 },
        ]);
        expect(report.sourceCanonicalConflicts.map((x) => ({
            domainId: x.domainId, pid: x.pid, uid: x.uid,
            expected: x.expected?.role || null, actual: x.actual?.role || null,
        }))).to.deep.equal([
            { domainId: 'system', pid: 12, uid: 120, expected: 'maintainer', actual: 'verifier' },
        ]);
    ...
    it('reports every uncleared fence and repairs an exact unbound ACL mutation to zero drift', async () => {
        ...
        const failed = await service.grantDirect(
            pair.domainId, pair.pid, pair.uid, 'maintainer', 1, 'acl-58',
        ).catch((error) => error);
        ...
        expect(report.aclMutationFences.map((fence) => ({
        ...
        const wrong = await service.repairAclMutation(pair, 'other-request')
            .catch((error: Error) => error);
        expect(wrong.message).to.contain('requestId mismatch');
        expect(repo.fences.has(pairKey(pair))).to.equal(true);
        expect(repo.problemLocks.has(pairKey(pair))).to.equal(true);
    ...
    it('routes an ERROR-claim-bound ACL mutation to exact problem-write-claim repair', async () => {
        ...
        repo.problemWriteClaims.set('system:59', {
            domainId: 'system', pid: 59, requestId: 'claim-59', actor: 1,
            operation: 'permit-update', state: 'error', lastError: 'crashed',
            createdAt: new Date(), updatedAt: new Date(),
        });
        const marker = {
            ...pair, requestId: 'acl-59', from: null, to: 'maintainer' as const,
            intent: {
                action: 'set-source' as const, sourceType: 'direct' as const, sourceId: 'direct',
                role: 'maintainer' as const, grantedBy: 1, note: '',
            },
        ...
        const error = await service.repairAclMutation(pair, 'acl-59')
            .catch((caught: Error) => caught);
    ...
    it('explicitly reports source-less legacy canonical rows without mutating ACL data', async () => {
        ...
        repo.canonical.set(pairKey(direct), {
            ...direct, role: 'maintainer', active: true, grantedBy: 7,
            grantedAt: new Date('2025-01-01T00:00:00.000Z'), viaContest: null, note: 'direct-old',
        });
        repo.canonical.set(pairKey(contest), {
            ...contest, role: 'verifier', active: true, grantedBy: 8,
            grantedAt: new Date('2025-02-01T00:00:00.000Z'), viaContest: 'contest-old', note: 'contest-old',
        });
        repo.mutationCalls = 0;
    ...
    it('repairs legacy canonical rows into provenance-faithful direct or contest sources', async () => {
        ...
        repo.canonical.set(pairKey(direct), {
            ...direct, role: 'verifier', active: true, grantedBy: 17,
            grantedAt: new Date('2025-03-01T00:00:00.000Z'), viaContest: null, note: 'direct-old',
        });
        repo.canonical.set(pairKey(contest), {
            ...contest, role: 'maintainer', active: true, grantedBy: 18,
            grantedAt: new Date('2025-04-01T00:00:00.000Z'), viaContest: 'contest-preserve', note: 'contest-old',
        });
    ...
    it('hard delete cleanup resumes fences and clears source, canonical, and legacy-only ACL state', async () => {
        ...
        repo.fences.set(pairKey(fencedPair), {
            ...fencedPair, requestId: 'interrupted-grant', from: null, to: 'verifier',
            intent: { sourceType: 'contest', sourceId: 'c9', role: 'verifier', grantedBy: 1, note: '' },
            completedSteps: [], lastError: 'process stopped',
            createdAt: new Date(), updatedAt: new Date(),
        });
