...
const files = execSync(`grep -rln 'type="checkbox"' ${ROOT} | grep -v 'ui/checkbox.tsx' | grep -v 'styles.css'`, { encoding: 'utf8' })
    .split('\n')
    .filter(Boolean);
...
function rewriteFile(file) {
    ...
    if (!src.includes("from '@/components/ui/checkbox'") && !src.includes('from "@/components/ui/checkbox"')) {
