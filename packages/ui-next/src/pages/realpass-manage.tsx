 ...
 */
import {
  BarChart3, ClipboardPaste, Pencil, Search, Trash2,
} from 'lucide-react';
...
export function RealPassManagePage() {
  ...
  const data = bs.page.data as {
    pdocs: R[]; page: number; ppcount: number; pcount: number; q: string;
  ...
  const saveSingle = async () => {
    ...
    try {
      await postOp({
        operation: 'set', target: target.trim(), accepted, submitted,
      });
  ...
  return (
    <div className="space-y-5">
      ...
      {msg ? (
        <div className={`rounded-md border px-3 py-2 text-sm ${msg.kind === 'err'
        ...
        <Card>
          <CardContent className="space-y-3 p-4">
            ...
            <div className="flex flex-wrap items-end gap-2">
              ...
              <Button onClick={saveSingle} disabled={busy}>保存</Button>
            </div>
            <p className="text-xs text-muted-foreground">
              纯数字 ID 会同时按题号(docId)与 pid 匹配；两者命中不同题时会拒绝并提示，请改用完整 pid。
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="space-y-3 p-4">
            <h2 className="flex items-center gap-1.5 text-sm font-semibold">
              <ClipboardPaste className="size-4" />批量粘贴导入
            </h2>
            <Textarea
              value={payload}
              readOnly={busy}
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" onClick={() => runBatch(false)} disabled={busy}>预览（不写入）</Button>
        ...
        <Card>
          <CardContent className="p-4">
            <h2 className="mb-2 text-sm font-semibold">
              批量预览 · 共 {preview.summary.total} 行：命中 {preview.summary.ok}
              {' '}· 未找到 {preview.summary.unmatched} · 冲突 {preview.summary.conflict}
              {' '}· 格式错误 {preview.summary.invalid} · 重复 {preview.summary.duplicate}
            </h2>
                    ...
                    return (
                      <TableRow key={i}>
                        ...
                        <TableCell className="text-xs">
                          ...
                          {r.status === 'ok' ? (
                            <span className="ml-2 text-muted-foreground">→ {r.pid || `#${r.docId}`} {r.title}</span>
                          ) : r.reason ? (
                            <span className="ml-2 text-red-600 dark:text-red-400">{r.reason}</span>
                          ) : null}
                        </TableCell>
                        <TableCell className="text-right font-mono text-xs">
                          {r.accepted != null ? `${r.accepted}/${r.submitted}` : '—'}
                        </TableCell>
                      </TableRow>
                    );
                  })}
          ...
          <CardContent className="p-4">
            ...
            <div className="space-y-1 text-xs text-muted-foreground">
              {(migration.unmatched || []).slice(0, 20).map((u: R, i: number) => (
                <div key={`u${i}`} className="font-mono">未匹配：{u._id} → {u.accepted}/{u.submitted}（可用上方单题录入手动补）</div>
              ))}
              {(migration.conflicts || []).slice(0, 20).map((c: R, i: number) => (
                <div key={`c${i}`} className="font-mono">冲突：{c._id} → docId #{c.byDocId} / pid #{c.byPid}，请人工裁决后用 pid 录入</div>
              ))}
              {(migration.unmatched?.length || 0) > 20 || (migration.conflicts?.length || 0) > 20 ? (
                <div>
                  （未匹配 {migration.unmatched?.length || 0} 条 / 冲突 {migration.conflicts?.length || 0} 条，
                  各仅显示前 20 条，完整清单在 system 集合 realpass.migration_unmatched）
                </div>
              ) : null}
            </div>
          </CardContent>
            ...
            <TableBody>
              {pdocs.length === 0 ? (
                ...
                <TableRow key={String(p.docId)}>
                  ...
                  <TableCell>
                    <a href={`/p/${p.pid || p.docId}`} className="text-sm hover:text-primary hover:underline">{p.title}</a>
                  </TableCell>
                  <TableCell className="text-right font-mono text-xs">
