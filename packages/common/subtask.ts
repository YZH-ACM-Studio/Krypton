...
interface MatchRule {
    regex: RegExp;
    output: (a: RegExpExecArray) => Array<string | null>;
...
const SubtaskMatcher: MatchRule[] = [
        ...
        output: (a) =>
            (
                ['out', 'ans']
                    ...
                    .flatMap((i) => [`${a[1]}.${i}`, `${a[1]}.${i}`.replace(/input/g, 'output').replace(/INPUT/g, 'OUTPUT')]) as Array<string | null>
    ...
    },
    {
        regex: /^(\D*)\.(in|IN)(\d+)$/,
        output: (a) =>
            [`${a[1]}.${a[2] === 'in' ? 'ou' : 'OU'}${a[3]}`, `${a[1]}.${a[2] === 'in' ? 'out' : 'OUT'}${a[3]}`].flatMap((i) => [
                i,
                i.replace(/input/g, 'output').replace(/INPUT/g, 'OUTPUT'),
            ]),
        id: (a) => +a[2],
        subtask: () => 1,
...
export function readSubtasksFromFiles(files: string[], config: ReadSubtasksConfig) {
    ...
    for (const file of files) {
        ...
        for (const rule of SubtaskMatcher) {
            ...
            const outputs = (config.noOutputFile ? ['/dev/null'] : rule.output(data)).filter((i): i is string => Boolean(i));
...
export function normalizeSubtasks(
    ...
    return subtasks.map((s, id) => {
        ...
        return {
            ...
            if: [],
            ...s,
            type: Object.hasOwn(s, 'type') ? s.type : 'min',
            score,
