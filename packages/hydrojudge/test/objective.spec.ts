...
describe('objective judge integration', () => {
    ...
    it('uses configured multi credit and rejects empty or wrong selections', async () => {
        expect((await run(multiConfig, { main: ['A'] })).result).to.deep.include({ status: STATUS.STATUS_WRONG_ANSWER, score: 35 });
        expect((await run(multiConfig, { main: ['A', 'C'] })).result).to.deep.include({ status: STATUS.STATUS_ACCEPTED, score: 100 });
        expect((await run(multiConfig, { main: ['A', 'B'] })).result).to.deep.include({ status: STATUS.STATUS_WRONG_ANSWER, score: 0 });
        expect((await run(multiConfig, { main: [] })).result).to.deep.include({ status: STATUS.STATUS_WRONG_ANSWER, score: 0 });
    });

    it('normalizes only blank CRLF and outer whitespace while preserving case', async () => {
        const config = { answers: { main: ['Answer\nLine', 100, { kind: 'blank' }] } };
        expect((await run(config, { main: '  Answer\r\nLine\n' })).result).to.deep.include({ status: STATUS.STATUS_ACCEPTED, score: 100 });
        expect((await run(config, { main: 'answer\nLine' })).result).to.deep.include({ status: STATUS.STATUS_WRONG_ANSWER, score: 0 });
    });
    ...
    it('uses the same one-line trim and case-sensitive rules for text program-fill', async () => {
        ...
        expect((await run(config, { main: 'I++' })).result).to.deep.include({ status: STATUS.STATUS_WRONG_ANSWER, score: 0 });
    });
    ...
    it('rejects array-shaped submissions for scalar objective problems', async () => {
        ...
        for (const result of results) {
            expect(result.result).to.deep.include({ status: STATUS.STATUS_WRONG_ANSWER, score: 0 });
        }
    });
});
