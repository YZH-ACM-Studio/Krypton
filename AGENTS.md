...
- Service 是持久锁唯一 owner；GUI 短 lease 与持久活动冲突时 fail closed。控制面断线或 Service 停止不得解锁，恢复时先验证签名状态再恢复规则；释放只允许签名 stop（含本机无活动且 WFP 为空时认领当前签名 stop）、硬截止或提升后的本机管理员 recovery。心跳 inactive、硬截止和本机 recover 都不得冒充这条 stop ACK。首次清理失败必须保持可观测的 release_pending 并重试，不得谎报 inactive；硬截止同时使用绝对 UTC 与本次进程的单调时钟，避免墙钟回拨延长活动。
...
- `exam.policyTemplates` 是学校范围内可复用的策略草稿容器；草稿可改，发布后的内嵌 revision 不可改。每个 revision 只保存 canonical 域名、IP/CIDR、端口与 fingerprint；发布与启动必须显式允许 Vigil 控制面 host/IP，隐式 service-only 许可只给 Endpoint Service，不能让学生 GUI、截图或 SRS 通。已填端口时还必须包含控制面端口和 `1935`。发布校验必须通过受信 resolver 取得本站 Endpoint Service 的实际控制面 host/port，并按终端相同的去重与端口展开语义把隐式 service-only 许可计入 4096 条规则上限。resolver 缺失或控制面无法解析时 fail closed。归档模板不得删除历史 revision，回滚只重新引用旧 revision，不生成反向 patch。
...
- Krypton 预登录 workflow 必须由服务端从已发布 assignment 派生 endpoint，并同时固定 preparation fingerprint、当前 active network execution revision、policy/target 完整引用、硬截止、目标覆盖和 P2.8 hard readiness。预登录 endpoint 必须全部包含在冻结 target 中，网络执行必须在硬截止前对完整目标真实 `applied`；USB/进程/检测器 warning 只展示且不进入 drift fingerprint，离线、版本/能力不兼容、目标缺失或检测不可用属于硬错误。终端预检不得把 Service 处于 Session 0、学生 GUI 未启动时的 `foreground_interactive_session_required`，以及部分系统进程路径读不到（`process_path_partial_*`）当成展示 warning；进程快照失败、USB、禁用进程和其它检测器失败仍展示。
...
- P2.13 在既有座位页显式开放 v2 教师工作流：老师选择一至多个当前教室后追加候选计划，再选择策略生成、跨教室换位、锁定、重新随机、保存不可变 revision 并通过 publication CAS 发布。页面只投影冻结的教室容量、风险边、拆队、跳过座位、Endpoint 在线 warning 与未设置朝向；风险边明细和连线按需展开，500 人只保留一个共享座位选择器并使用 Map/Set 连接，不为每行复制完整座位控件。任何未保存的映射或锁定变化都必须阻止发布、CSV 导出和候选计划 mutation；请求在途、响应未知、重读失败或 assignment 不再精确引用当前最新 plan 时页面必须标记事实过期并关闭全部写入/导出，老师只能显式放弃未保存草稿后重读，禁止猜测写入结果。完全公开或仅靠邀请码且参赛者仍可变化的个人赛第一版不提供自动排座，也不得用学校、用户组或瞬时 attend 绕过；只有固定受众或已定版团队名单可进入 v2 写边界。CSV 必须携带教室/座位/Endpoint 身份并中和公式。v1 历史继续只读兼容，P2.13 不修改长期绑定或运行档案；v2 预登录消费者只由后续 P2.14 兼容 reader/writer 接入。
...
- P2.14 复用同一座位页和既有网络目标、网络执行、监测预检、预登录 batch/ticket/projection/retry canonical，不增加 Client/Vigil wire 字段、第二套批次、调度器、Wake-on-LAN 或自动唤起。比赛管理页只为固定个人受众或已定版团队提供 ExamEvent 创建/选择入口；个人用户组受众必须由现有 group→school 目录完整解析到同一学校，缺组或跨校范围第一版明确拒绝并要求拆分比赛，不得创建一个必然跨校失败的 ExamEvent。零个活动需按比赛名称、学校和时间显式创建，一个活动直接进入，多个活动必须显式选择。教师在同一页按七步完成冻结名单、选择候选教室与朝向/禁用事实、选择策略并生成、解释/换位/锁定、保存并 publication CAS、终端/监测/网络预检与显式网络启动、最后显式一键预启动；10–15 分钟仅为建议，不产生定时任务。
- P2.14 在任何 v2 target 解析、prepare、新 confirm 或 retry 前共享重读当前比赛受众、名单、计划、教室 layout/运行档案和 active binding：Krypton v2 自动排座与预登录只接受同一 Contest 的 canonical `contestAudience` 名单，v2 userbind 学校/用户组来源仅保留历史只读，不得用子集漏掉参赛者；既有 v1 userbind、公开/邀请码票据与预登录兼容路径继续按 P2.6 规则读取和重验，不得被 v2 门禁误伤。比赛变为公开/仅邀请码、名单/禁用/座位删除、bindingId/revision/endpoint 漂移是 v2 整批硬错误，禁止部分投递或猜测替代座位。仅朝向变化是 `seat_facing_changed` warning，仍使用已发布分配，且该展示 warning 不得进入 preparation 的授权指纹或使已发行票据失效。已经按同一 requestId 持久 claim 的 confirm 必须继续用冻结 batch/ticket 精确收敛，不能因响应丢失后的 live drift 改写或拒绝既有请求；v2 batch 在部分票据写入后崩溃时，从 batch 精确引用的不可变 v2 assignment、canonical Contest roster 与确定性 ticketId 重建尚缺的既有 ticket 字段，恢复禁止回读当前 publication、binding、受众或其它 live readiness。P2.14 不扩展既有 batch/ticket shape，也不得建立第二套票据 canonical。内部 join 必须使用 `(classroomId,sourceSeatId)`，票据继续保存既有最小 `sourceSeatId` 与精确 binding/endpoint 身份，因此不同教室同名座位不得碰撞。P2.14 reader 只能在 `exam.preloginV2WriterEnabled=false` 时先部署并验证后显式启用；启用后允许关闭 writer，但存在活动 v2 batch 时禁止回滚到不支持 P2.14 v2 消费者的旧制品。
...
- 已经 inactive 且仍持有匹配的最后 stop proof 时，才能按原规则幂等接受同一 activity/policyRevision 的更高 command revision stop。本机没有当前活动、托管过滤器为 0，且最后 stop proof 缺失或与新命令不一致时，必须认领这条新的签名 stop，写成 `authorized_stop_empty_owner`，回显该命令的 activity/revision/commandId；这是整盘还原后的正规结清路径，不是心跳 inactive 或硬截止冒充 ACK。WFP 仍有托管规则、release_pending 未完成、或存在 legacy lease 时必须 fail closed。重复 stop 的 pending proof 必须按持久 pending → replay commit → inactive 顺序恢复，不能清除无关 legacy 网络规则。
- 新执行会话若上报 `inactive`、0 条托管规则、且该终端最新网络命令仍是未过硬截止的成功 apply，Vigil 必须用同一活动授权再发一次更高 command revision 的 apply；最新命令已是任意状态的 stop、硬截止已过、或本机仍有托管规则时不得补发。这是在兑现已持久的 apply intent，不新建 OJ execution revision，也不把心跳 inactive 记成 stop ACK。
