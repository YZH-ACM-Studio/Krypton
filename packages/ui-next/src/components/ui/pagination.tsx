...
export function Pagination({ current, total, baseUrl }: { current: number; total: number; baseUrl: string }) {
    ...
    <nav className="flex items-center justify-center gap-1 pt-4">
      ...
      {pages.map((p, i) =>
        p === '...' ? (
          ...
          <Button key={p} asChild variant={p === current ? 'default' : 'ghost'} size="icon">
            <a href={href(p)}>{p}</a>
          </Button>
        ),
      )}
