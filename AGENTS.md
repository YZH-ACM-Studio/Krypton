...
- 一旦同一事件出现 v2 seat-plan 或 assignment，所有 v1 生成、调整、重新随机和发布入口必须 fail closed，禁止在 v2 历史后追加 v1。任何 v2 写入都会把二进制回滚下限永久提升到能严格读取 v2 union 的兼容 reader，禁止回到只认识 v1 exact-key 的旧制品。
